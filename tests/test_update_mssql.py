...
def test_update_data(mocker):
    ...
    sheet_config = sheet_config = {"table": "test", "key_column": "id_registro"}
    ...
    assert (
        "staging_test" in table_name_passed_to_sql
    )  # Verifica se o nome da tabela de staging está correto
    ...
    assert (
        "UPDATE SET T.[dados] = S.[dados], T.[dados2] = S.[dados2]" in merge_sql_query
    )
