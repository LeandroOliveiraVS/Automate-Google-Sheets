...
def fetch_data(gcp_conn_id: str, sheet_config:dict) -> pd.DataFrame:
    ...
    try:
        ...
        logging.info(f"Obtendo credenciais do Google a partir da conexão: {gcp_conn_id}")
        ...
        for col in df.columns:
            if df[col].dtype == 'object':
                df[col] = df[col].astype(str).replace('nan', '') # Converte e limpa possíveis 'nan' de texto
        return df
