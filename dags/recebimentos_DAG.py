...
def dag_processar_recebimentos():
    ...
    def extract_data_task(sheet_config: dict) -> pd.DataFrame:
        df_extraido = extract.fetch_data(
            gcp_conn_id="google_cloud_default", sheet_config=sheet_config
        )
    ...
    def transform_data_task(
        sheet_config: dict, df_extraido: pd.DataFrame
    ) -> pd.DataFrame:
        df_transformado = transform.Transform_Data_1(
            sheet_config=sheet_config, df_extraido=df_extraido
        )

        return df_transformado

    @task
    def compare_data_task(
    ...
    def load_data_task(mssql_conn_id: str, sheet_config: dict, df_final: pd.DataFrame):
        load.load_data_to_mssql(
            mssql_conn_id=mssql_conn_id, sheet_config=sheet_config, df_final=df_final
        )
